...
public class DocumentsAdapter extends RecyclerView.Adapter<BaseHolder> {
    ...
    public void swapResult(DirectoryResult result) {
        ...
        if (extras != null) {
            ...
            if (info != null) {
                mFooters.add(new AvMessageFooter(mEnv, ITEM_TYPE_INFO, R.drawable.ic_dialog_info, info));
            }
            final String error = extras.getString(DocumentsContract.EXTRA_ERROR);
            if (error != null) {
                mFooters.add(new AvMessageFooter(mEnv, ITEM_TYPE_ERROR, R.drawable.ic_dialog_alert, error));
            }
            if (extras.getBoolean(DocumentsContract.EXTRA_LOADING, false)) {
                mFooters.add(new AvLoadingFooter(mEnv, ITEM_TYPE_LOADING));
            }
        }

        if (result != null && result.exception != null) {
            mFooters.add(new AvMessageFooter(mEnv, ITEM_TYPE_ERROR, R.drawable.ic_dialog_alert, getString(R.string.query_error)));
        }

        mHeader = new AvMessageFooter(mEnv, ITEM_TYPE_HEADER, R.drawable.ic_doc_folder,
                getTitle());
        mEnv.setEmptyState();
    ...
    public BaseHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        switch (viewType) {
            ...
            case ITEM_TYPE_LOADING: {
                return new AvLoadingHolder(mEnv, mEnv.getContext(), parent);
            }
            case ITEM_TYPE_INFO:
            case ITEM_TYPE_ERROR: {
                return new AvMessageHolder(mEnv, mEnv.getContext(), parent);
            }
            case ITEM_TYPE_HEADER: {
                return new AvMessageHolder(mEnv, mEnv.getContext(), parent, R.layout.item_message_header);
            }
        }
        return null;
    }
